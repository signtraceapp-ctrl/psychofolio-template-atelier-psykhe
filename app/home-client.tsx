...
import { Feather, Quote, ArrowRight } from "lucide-react";
...
export function HomeClient({ content: c }: { content: SiteContent }) {
  return (
    <div className="font-sans selection:bg-primary/20 bg-bg text-fg">
      {/* Hero - centered, editorial, museum-like */}
      <section className="relative overflow-hidden py-24 md:py-36">
        <div className="absolute inset-0 -z-10 bg-gradient-to-b from-primary/5 via-bg to-bg" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -z-10 h-[500px] w-[500px] rounded-full bg-primary/3 blur-[150px]" />
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="inline-flex items-center gap-2 text-[10px] tracking-[0.3em] uppercase text-primary/80">
              <Feather className="h-3.5 w-3.5" /> {c.home.badge}
            </div>
            ...
            <div className="pt-4">
              <a
                href="/iletisim"
                className="inline-flex items-center justify-center gap-2 border border-primary/40 px-10 py-3.5 text-xs tracking-[0.15em] uppercase font-medium transition-all duration-300 hover:bg-primary hover:text-primary-fg text-primary"
              >
                {c.home.cta}
      ...
      <section className="py-24 border-y border-border/30">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            <Quote className="h-8 w-8 text-primary/25 mx-auto" />
      ...
      {/* Services - gallery-style cards with thin borders */}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="space-y-14">
            ...
            <motion.div
              ...
              whileInView="animate"
              viewport={{ once: true }}
              className="grid gap-px sm:grid-cols-2 border border-border/30"
            >
              {c.services.map((s, i) => (
                <motion.div
                  key={i}
                  variants={fadeUp}
                  className="group p-8 bg-bg hover:bg-bg-secondary/50 transition-colors duration-500 border border-border/30"
                >
      ...
      <section className="py-24 bg-bg-secondary/30">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="space-y-14">
            ...
            <div className="mx-auto max-w-2xl divide-y divide-border/30">
              {c.articles.map((a, i) => (
                <motion.div
                  key={i}
                  initial={{ opacity: 0 }}
