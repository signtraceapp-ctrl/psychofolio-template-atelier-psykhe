      ...
      <body className="min-h-screen flex flex-col bg-bg text-fg antialiased">
        ...
        <header className="sticky top-8 z-50 border-b border-border/40 bg-bg/90 backdrop-blur-md">
          <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-5 sm:px-6 lg:px-8">
            <Link href="/" className="font-display text-xl font-light tracking-wide text-primary hover:text-primary-hover transition-colors">
              {c.site.name}
            </Link>
            <nav className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-xs tracking-[0.15em] uppercase text-fg-muted hover:text-primary transition-colors"
                >
                  {link.label}
                </Link>
              ))}
        ...
        <footer className="border-t border-border/40 bg-bg-secondary/50 py-10">
          <div className="mx-auto max-w-7xl px-4 text-center text-sm text-fg-muted sm:px-6 lg:px-8">
            <p className="font-display text-base text-fg font-light">{c.site.name}</p>
            <p className="mt-1 text-xs tracking-wider">{c.site.title}</p>
